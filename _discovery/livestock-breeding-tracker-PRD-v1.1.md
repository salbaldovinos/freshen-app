# Kindled — Product Requirements Document
**Version:** 1.1 | **Status:** Approved | **Last Updated:** 2026-03-18  
**Scope:** MVP → v1.0 production launch on iOS and Android  
**Primary build tool:** Claude Code  
**App name:** Kindled | **Bundle ID:** `com.kindledapp.kindled` | **Domain:** kindledapp.com
...
export const breedingRecords = sqliteTable('breeding_records', {
  ...
  color: text('color').notNull().default('gray'),       // one of 8 preset hex values
...
calculateDueDate("2026-01-01", 150) → "2026-05-31"  ← correct: verify this with js
...
**Note for MVP:** The species picker is included in the form but only "Goat" is shown in the MVP build. The config is built out now so the v1.0 multi-species unlock is trivial to implement.
...
- [ ] Write db/schema.ts (breedingRecords, births tables)
...
2. Welcome screen shows: Kindled logo, tagline `APP_TAGLINE` ("Every new life starts here."), two buttons: "Get Started Free" and "Sign In"
...
- Headline: "Upgrade to Kindled Pro"
...
Users set reminder preferences in Settings. Defaults:
...
- `kindled_monthly` — $1.99/month
- `kindled_annual` — $9.99/year (with 7-day free trial)
- `kindled_lifetime` — $24.99 one-time
...
PostHog provides product analytics to understand how users interact with Kindled. All tracking calls are routed through `lib/analytics.ts` — no component ever calls PostHog directly.
...
File name: `kindled-export-YYYY-MM-DD.csv`

**PDF layout:**

- Header: Kindled logo wordmark + "Breeding Records Export" + export date
...
- Footer: "Generated by Kindled · kindledapp.com · [date]"
- Page numbers: "Page N of M"

File name: `kindled-export-YYYY-MM-DD.pdf`
...
And the file is named kindled-export-[today's date].pdf
...
export const breedingFormSchema = z.object({
  ...
  color: z.enum(['gray', 'red', 'orange', 'yellow', 'green', 'teal', 'blue', 'purple']),
});
```
...
App Store prep
- [ ] Create privacy policy page at kindledapp.com/privacy covering all data collected
...
| **App name** | **Kindled** — `com.kindledapp.kindled` / `kindledapp.com`. Name is isolated in `constants/app.ts`; renaming requires editing only that file and `app.json`. Full naming research and trademark clearance rationale documented separately. |
...
## Changelog
- 2026-03-18 v1.1: Added app name (Kindled), name-change isolation policy, constants/app.ts spec, PostHog analytics (Feature 2.7), data export CSV+PDF (Feature 2.8), PostHog integration doc, confirmed photo limit (1 per breeding record with Replace/Remove flows), confirmed notification permission timing (post-signup), resolved all open questions, updated privacy labels for analytics, updated task checklists
